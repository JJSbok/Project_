...
<form action="/member/update" method="post" name="updateForm">
  ...
  <input type="button" value="수정" onclick="update()"> <%-- onclick="update()"은 아래 update함수 실행 --%>
<%--input type="submit"의 경우 해당 페이지로 바로 제출됨 button은 다른 기능을 줌 --%>
...
<script>
  const update = () => {
    ...
    if (passwordDB == password) {
      document.updateForm.submit(); //일치하면 updateForm 폼태그를 실행 하겠다
    } else {
